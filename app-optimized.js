...
let currentView = localStorage.getItem('currentView') || 'all'; // Track current view: 'all', 'premium', 'fav'
    ...
    if (searchInput) {
        searchInput.addEventListener('input', (e) => {
            ...
            if (currentView === 'premium') {
                ...
                filtered = allCombined.filter(ch => 
...
// Load More Channels (Lazy Loading)
function loadMoreChannels() {
    ...
    channelsToAdd.forEach(ch => {
        ...
        card.innerHTML = `
            <img src="${ch.logo}" loading="lazy" onerror="this.src='https://via.placeholder.com/300x150/181818/E50914?text=${encodeURIComponent(ch.name)}'">
...
// Optimized Display with Lazy Loading
...
function filterCategory(cat) {
    ...
    displayChannelsLazy(cat === 'all' ? allChannels : allChannels.filter(ch => ch.category === cat));
}
...
function showPremiumLive() {
    ...
    localStorage.setItem('currentView', 'premium');
...
function showFavorites() {
    ...
    localStorage.setItem('currentView', 'fav');
    document.getElementById('section-title').innerText = 'My List';
    const allCombined = [...allChannels, ...premiumChannels];
    displayChannelsLazy(allCombined.filter(ch => favorites.includes(ch.url)));
}
